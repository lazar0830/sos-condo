...
const EditServiceProviderUserModal: React.FC<EditServiceProviderUserModalProps> = ({ provider, users, onClose, onSave }) => {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title={isEditing ? 'Edit Service Provider' : 'Add New Service Provider'}>
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* User Account Section */}
        <fieldset className="border border-gray-300 dark:border-gray-600 p-4 rounded-md">
            <legend className="px-2 text-lg font-semibold text-gray-800 dark:text-gray-100">User Account</legend>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                ...
                {!isEditing && (
                    <div className="md:col-span-2">
                        ...
                        <input type="password" name="password" id="password" value={userData.password} onChange={handleUserChange} className="mt-1 block w-full input" placeholder={isEditing ? 'Leave blank to keep current' : ''} required={!isEditing} />
                    </div>
                )}
            </div>
        </fieldset>
        ...
        {/* Provider Profile Section */}
         <fieldset className="border border-gray-300 dark:border-gray-600 p-4 rounded-md">
            <legend className="px-2 text-lg font-semibold text-gray-800 dark:text-gray-100">Provider Profile</legend>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                <div>
